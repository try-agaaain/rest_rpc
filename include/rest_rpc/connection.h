...
private:
  void read_head() {
    std::cout << "1. read_head > 开始 reset_timer" << std::endl;
    reset_timer();
    ...
    async_read_head([this, self](asio::error_code ec, std::size_t length) {
      ...
      if (!ec) {
        ...
        if (body_len > 0 && body_len < MAX_BUF_LEN) {
          ...
          std::cout << "2. async_read_head > 开始 read_body" << std::endl;
          read_body(header->func_id, body_len);
          return;
        }

        if (body_len == 0) { // nobody, just head, maybe as heartbeat.
          std::cout << "body_len == 0 > 开始 cancel_timer" << std::endl;
          cancel_timer();
          std::cout << "body_len == 0 > 开始 read_head" << std::endl;
          read_head();
        } else {
          ...
          std::cout << "body_len != 0 > 开始 close" << std::endl;
          close();
        }
      } else {
        print(ec);
        ...
        std::cout << "-1. ec != 0 > 开始 close" << std::endl;
        close();
      }
    });
  }
  ...
  void read_body(uint32_t func_id, std::size_t size) {
    auto self(this->shared_from_this());
    std::cout << "3. read_body > 开始 async_read" << std::endl;
    async_read(size, [this, func_id, self](asio::error_code ec,
                                           std::size_t length) {
      std::cout << "4. async_read > 开始 cancel_timer" << std::endl;
      cancel_timer();
      ...
      if (!ec) {
        std::cout << "5. ec == 0 > 开始 read_head" << std::endl;
